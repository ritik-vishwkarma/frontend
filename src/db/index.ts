...
const API_BASE_URL = 'http://localhost:3000/connection';
...
export async function fetchPosts(): Promise<ApiPost[]> {
    try {
        const response = await fetch(`${API_BASE_URL}/samples/collection_2025_03_18t04_09_32_036z_hxpa89q06`);
        if (!response.ok) {
...
export function processPostsByDay(posts: ApiPost[]): DayData[] {
   ...
   */
  export async function fetchTimeEngagementData() {
    try {
      const posts = await fetchPosts();
      return {
